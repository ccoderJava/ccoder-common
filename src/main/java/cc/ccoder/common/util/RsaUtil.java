 ...
 * @date RsaUtil.java v1.0  2021/11/1 17:12
 */
public class RsaUtil {
    ...
    public static RsaKeyPair generateKeyPair(int keySize) {
        ...
        return new RsaKeyPair(Base64.getEncoder().encodeToString(keyPair.getPrivate().getEncoded()),
     ...
     *
     * @param plain      签名原文
     * @param charset    原文字符集编码
     * @param privateKey 私钥
     * @return base64(签名)
     */
    public static String sign(String plain, Charset charset, java.security.PrivateKey privateKey)
            throws SignatureException, InvalidKeyException {
        return Base64.getEncoder().encodeToString(sign(plain.getBytes(charset), privateKey));
    }
     ...
     * @return 是否验签通过
     */
    public static boolean verifySign(String plain, Charset charset, String sign, java.security.PublicKey publicKey)
            throws SignatureException, InvalidKeyException {
        return verifySign(plain.getBytes(charset), Base64.getDecoder().decode(sign), publicKey);
    }
     ...
     * @param data       明文
     * @param publicKey  公钥
     * @param rsaKeySize rsa密钥大小，典型的：2048，4096
     * @return 密文
     */
     ...
     *
     * @param data       字符串明文
     * @param charset    明文字符集编码
     * @param publicKey  公钥
     * @param rsaKeySize rsa密钥大小，典型的：2048，4096
     * @return base64(密文)
     */
    public static String encrypt(String data, Charset charset, java.security.PublicKey publicKey, int rsaKeySize) throws Exception {
        return Base64.getEncoder().encodeToString(encrypt(data.getBytes(charset), publicKey, rsaKeySize));
    }
     ...
     * @return 字符串明文
     */
    public static String decrypt(String encryptedData, Charset charset, java.security.PrivateKey privateKey, int rsaKeySize)
            throws Exception {
        return new String(decrypt(Base64.getDecoder().decode(encryptedData), privateKey, rsaKeySize), charset);
    }
